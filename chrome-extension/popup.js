...
function showError(msg) {
    ...
    el.textContent = msg;
    el.style.cssText = 'position:fixed;top:8px;left:8px;right:8px;padding:10px;background:rgba(239,68,68,0.95);color:white;border-radius:12px;font-size:12px;font-weight:500;z-index:999;text-align:center;backdrop-filter:blur(8px);box-shadow:0 4px 16px rgba(0,0,0,0.3);animation:fadeIn 0.2s ease;';
...
async function loadApiToken() {
    try {
        const res = await fetch(API + '/api/token', { signal: AbortSignal.timeout(2000) });
...
function renderStatus(data) {
    ...
    if (active) {
        ...
        if (infoMode) {
            infoMode.textContent = data.session_type === 'rescue' ? 'Rescue Throne 🛡️' : data.mode;
        }
